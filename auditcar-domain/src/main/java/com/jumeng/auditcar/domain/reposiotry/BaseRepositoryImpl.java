package com.jumeng.auditcar.domain.reposiotry;

import com.jumeng.auditcar.domain.reposiotry.support.*;
...
//		如果排序内容为空 则执行不排序的 查找
        if(dataQueryObjectSort.getPropertyName() != null && dataQueryObjectSort.getPropertyName().trim().length() != 0) {
            return this.findAll(dqo, Sort.by(new Order(dataQueryObjectSort.isAscending() ? Direction.ASC : Direction.DESC, dataQueryObjectSort.getPropertyName())));
        } else {
    ...
    public Page<T> findAll(DataQueryObjectPage dataQueryObjectpage) {
        ...
        if(dataQueryObjectpage.getPropertyName() != null && dataQueryObjectpage.getPropertyName().trim().length() != 0) {
            pageable = PageRequest.of(dataQueryObjectpage.getPage(), dataQueryObjectpage.getSize(), Sort.by(new Order(dataQueryObjectpage.isAscending() ? Direction.ASC : Direction.DESC, dataQueryObjectpage.getPropertyName())));
        } else {
            pageable = PageRequest.of(dataQueryObjectpage.getPage(), dataQueryObjectpage.getSize());
        }
